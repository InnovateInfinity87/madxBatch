###################################################################
#                  DATAPROCESSING FOR PYTHON 3                    #
# This version of "dataprocessing" has been converted to Python3: #
# the compatibility with Python2 is broken.                       #
#                                                                 #
# Since this code was not importing other madxBatch modules, the  #
# conversion has not been particularly painful.                   #
#                                                                 #
# Modifications:                                                  #
#    - 2to3 suggested modifications implemented as in             #
#      "changesToDataProcessing.txt" file.                        #
#    - iteritems replaced everywhere with items.                  #
#    - lrange replaced everywhere with list(range()).             #
#    - removed import of iteritems and lrange.                    #
#                                                                 #
# NOTICE: inconsistencies with SciPy modules from                 #
#         Python2 to Python3 have not been checked.               #
#                                                                 #
# M.Pari                                                          #
###################################################################
...
_sidict = {18: 'E', 15: 'P', 12: 'T',
           ...
           -3: 'm', -6: '$\mu$', -9: 'n',
           -12: 'p', -15: 'f', -18: 'a'}
_sidict_rev = {value: key for key, value in _sidict.items()}
...
def lossmap(data, twiss, slim=None, merge=True, threshold=0.0001,
    ...
    fig, ax = plt.subplots()
    xvals = list(range(1, len(mydata)+1))
...
def lossmapscan(lossdict, twiss, slim=None, merge=True, apmerge=True,
    ...
    alldata = None
    for scanparam, data in lossdict.items():
